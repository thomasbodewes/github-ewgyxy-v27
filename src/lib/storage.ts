...
const STORAGE_KEYS = {
  ...
  QUESTIONNAIRES: 'medical_questionnaires',
} as const
...
export class StorageManager {
  ...
  getMedicalQuestionnaire(patientId: string): MedicalQuestionnaire | null {
    const questionnaires = this.getItem<MedicalQuestionnaire[]>(STORAGE_KEYS.QUESTIONNAIRES, [])
    return questionnaires.find(q => q.patientId === patientId) || null
  }

  saveMedicalQuestionnaire(questionnaire: MedicalQuestionnaire): void {
    const questionnaires = this.getItem<MedicalQuestionnaire[]>(STORAGE_KEYS.QUESTIONNAIRES, [])
    const index = questionnaires.findIndex(q => q.patientId === questionnaire.patientId)
    
    if (index >= 0) {
      questionnaires[index] = { ...questionnaire, updatedAt: new Date().toISOString() }
    } else {
      questionnaires.push({
        ...questionnaire,
        createdAt: new Date().toISOString(),
    ...
    this.setItem(STORAGE_KEYS.QUESTIONNAIRES, questionnaires)
  }
